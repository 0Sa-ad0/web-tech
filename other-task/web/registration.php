...
<body>
    <?php
    $draft_data = []; // Initialize the $draft_data array

    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['save_draft'])) {
            $firstname = isset($_POST["firstname"]) ? $_POST["firstname"] : "";
            $lastname = isset($_POST["lastname"]) ? $_POST["lastname"] : "";
            // Code to handle saving draft data (if needed) goes here
            ...
            $required_fields = ["firstname", "lastname", "father'sname", "mother'sname", "gender", "bloodgroup", "Religion", "Email", "Phone/Mobile", "Website", "country", "Division", "rsc", "postcode", "username", "password", "confirmpassword"];
            ...
            if (!filter_var($website, FILTER_VALIDATE_URL)) {
                $errors[] = "The website URL is not valid.";
            }

            if (strlen($username) > 5 || !ctype_alnum($username)) {
                $errors[] = "The username should be less than 5 characters and consist of alphanumeric characters only.";
            }

            if (strlen($password) < 3 || $password !== $confirm_password) {
                $errors[] = "The password must be at least 3 characters and match the confirmation password.";
            }
            ...
            if (!empty($errors)) {
                ...
                // Perform header redirection before any output is sent
    ...
    function preFill($field)
    {
        ...
        } elseif (isset($_COOKIE['draft_data']) && isset(unserialize($_COOKIE['draft_data'])[$field])) {
            return htmlspecialchars(unserialize($_COOKIE['draft_data'])[$field]);
        } else {
            return "";
        }
    }
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="application/x-www-form-urlencoded" autocomplete="on" novalidate>
        ...
        <?php
        if (isset($_POST['save_draft'])) {
            echo "SAVE AS DRAFT"; // Display the message for "Save as Draft"
        }
        if (!empty($draft_data)) {
